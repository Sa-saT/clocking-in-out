...
- [ ] 12. Vercelへのデプロイ
  - [x] 12-1. GitHubリポジトリの作成・連携
  - [x] 12-2. nuxt.config.tsのVercel対応確認（特別な設定は不要、Zero Config）
  - [x] 12-3. Vercelアカウント作成・ログイン
  - [ ] 12-4. Vercelで「New Project」→GitHubリポジトリをインポート
  - [ ] 12-5. ビルドコマンド・出力ディレクトリは自動検出（特別な指定不要）
  - [ ] 12-6. 必要に応じて環境変数（例：DATABASE_URL）をVercelの設定画面で追加
  - [ ] 12-7. 「Deploy」ボタンでデプロイ実行
  - [ ] 12-8. 公開URLでアプリ・APIの動作確認
  - [ ] 12-9. 問題があれば修正→pushで自動再デプロイ
...
これにより、テストが不足している箇所（未カバーの関数・分岐・ファイル等）を一目で把握でき、品質向上・保守性向上に直結します。
---------------------------------------------------------
