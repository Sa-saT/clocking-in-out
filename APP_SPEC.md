...
- [ ] 11. 自動テスト（Vitest等によるユニット・コンポーネント・APIテスト）
  ...
  - [ ] 11-4. テストカバレッジの可視化・CI連携（Vercel単体運用）
    ...
    - [ ] 11-4-4. CI/CD自動化（最小構成：バッジ無し）
      - [ ] 11-4-4-1. GitHub Actionsでテスト・カバレッジ自動実行ワークフロー作成
...
- [ ] 13. Vercelへのデプロイ
  ...
  - [ ] 13-4. Vercelで「New Project」→GitHubリポジトリをインポート
  - [ ] 13-5. ビルドコマンド・出力ディレクトリは自動検出（特別な指定不要）
  - [ ] 13-6. 必要に応じて環境変数（例：DATABASE_URL）をVercelの設定画面で追加
  - [ ] 13-7. 「Deploy」ボタンでデプロイ実行
  - [ ] 13-8. 公開URLでアプリ・APIの動作確認
  - [ ] 13-9. 問題があれば修正→pushで自動再デプロイ
